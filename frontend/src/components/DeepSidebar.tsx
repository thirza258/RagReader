...
const DeepSidebar: React.FC<DeepSidebarProps> = ({ file, conversationId, documentId, onBack, onAnalyze }) => {
  ...
  // const handleGroundTruthChunkClick = () => {
  //   if (documentId) {
  //     navigate(`/ground-truth/${documentId}?conversationId=${conversationId}`);
  //   } else {
  //     alert("No document found for ground truth selection.");
  //   }
  // }

  // const handleGroundTruthResponseClick = () => {
  //   if (conversationId) {
  //     navigate("/ground-truth-response", { state: { conversationId } });
  //   }
  //     else {
  //     alert("No conversation found for ground truth response.");
  //   }
  // }
    ...
    <div className="w-1/3 min-w-[320px] max-w-[400px] h-full flex flex-col border-r border-[hsl(var(--border))] bg-[hsl(var(--card))] text-[hsl(var(--card-foreground))] shadow-2xl z-20">
      ...
      <div className="space-y-4">
        ...
        <section className="w-full">
          <div className="flex items-center gap-2 mb-3 text-[hsl(var(--primary))]">
            <Cpu size={16} />
            <h3 className="text-xs font-bold uppercase tracking-wider">Analysis Settings</h3>
          </div>
          <div className="flex items-center gap-2 mb-3 text-[hsl(var(--primary))]">
            <button
              onClick={() => stopAllWebSockets([])}
          ...
          {/* <div className="mb-2">
            <button
              onClick={handleGroundTruthChunkClick}
              className="flex items-center gap-2 px-3 py-2 rounded-md bg-blue-500 text-white hover:bg-blue-600 transition"
            >
              <Play size={16} />
              Choose Ground Truth Chunk
            </button>
          </div>

          <div>
            <button
              onClick={handleGroundTruthResponseClick}
              className="flex items-center gap-2 px-3 py-2 rounded-md bg-green-500 text-white hover:bg-green-600 transition"
            >
              <Play size={16} />
